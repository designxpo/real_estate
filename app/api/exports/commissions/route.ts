...
export async function GET(req: Request) {
  try {
    ...
    const splits = await prisma.commissionSplit.findMany({
      ...
      include: { user: { select: { name: true } }, deal: { select: { property: { select: { title: true } }, marketplaceListing: { select: { title: true } } } } },
    ...
    if (view === "tds") {
      ...
      const csv = toCsv(tdsRows, [
        ...
        { header: "Deal", value: (s) => s.deal.property?.title ?? s.deal.marketplaceListing?.title ?? "" },
